...
async function profile(auth) {
    ...
    const response = await axios.get('https://my.replika.ai/api/mobile/1.4/personal_bot', { headers: user_headers });
    return response.data;
}


async function chat(auth) {
    const user_headers = {
        ...base_headers,
        ...auth,
    };
    const response = await axios.get('https://my.replika.ai/api/mobile/1.4/personal_bot_chat', { headers: user_headers });
    return response.data;
}


async function user_profile(auth) {
    const user_headers = {
        ...base_headers,
        ...auth,
    };
    const response = await axios.get('https://my.replika.ai/api/mobile/1.4/profile', { headers: user_headers });
    return response.data;
}
